  ...
  const fetchSuppliers = async () => {
    ...
    try {
      ...
      if (supplierSummaries.length === 0) {
        // Usar dados mock se não houver registros no Supabase
        // Tipando corretamente os valores mock para SupplierTypeCode
        setSuppliers(mockSuppliers.map(supplier => ({
          ...supplier,
          supplierType: supplier.supplierType as any
        })))
      } else {
        // Garantir a tipagem correta do supplierType
        setSuppliers(supplierSummaries.map(supplier => ({
          ...supplier,
          supplierType: supplier.supplierType as any
        })))
      }
