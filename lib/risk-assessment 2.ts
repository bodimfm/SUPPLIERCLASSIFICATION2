...
// Função para determinar o nível de risco com base no tipo de fornecedor
export function getRiskLevel(supplierType: SupplierTypeCode): string {
  const riskLevels: Record<SupplierTypeCode, string> = {
    "A": "high",
    "B": "high",
    "C": "medium",
    "D": "low"
  }
  return riskLevels[supplierType];
}

// Função para calcular o tipo de fornecedor com base na matriz de risco
export function calculateSupplierType(dataVolume: DataVolume, dataSensitivity: DataSensitivity, contractType: "punctual" | "continuous" = "continuous"): SupplierTypeResult {
